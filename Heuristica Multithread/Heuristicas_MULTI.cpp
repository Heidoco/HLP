...
{ /// Função usada para recuperar os índices das variáveis z_km, m!=k
    if (k < m)
    {
        return (n - 1) * k + m - 1;
    }
...
{ // Leitura do conjunto de dados AP
    ifstream arq(name);
    if (!arq)
    {
        cerr << "Erro arquivo \n";
    ...
    for (int i = 0; i < n; i++)
    { // Coletar as coordenadas dos nós
        arq >> codx[i];
        arq >> cody[i];
    }
    ...
    for (int i = 0; i < n; i++)
    { // Coletar custos fixos instalação hub AP
        arq >> s[i];
        s[i] = s[i]/10;
        soma = soma + s[i];
    }
    ...
    for (int i = 0; i < n; i++)
    { // Custos de operação link entre hubs AP
        for (int j = 0; j < n; j++)
        {
            g[i][j] = 0.1 * (soma/n);
            q[i][j] = g[i][j] * 0.20;
...
{ // Solução inicial
    ...
    double quantidade_aux = 0.0;
    double f_avaliacao = 0.0;
    vector<vector<double>> l(n, vector<double>(n));
    vector<vector<double>> le(n, vector<double>(n));
    ...
    vet_sol_um_hub[0] = hub_ativo; // ALTERAR DEPOIS NAO RETORNAR, MANTER O VETOR
 ...
 //cout << "quantidade: " << quantidade << endl;
   ...
   // cout << "f_avaliacao_inicial " << f_avaliacao << endl;
...
{
    vector<vector<double>> hub_net_sp_matrix(n, vector<double>(n)); // Matriz de caminhos mais curtos apenas entre hubs
    vector<vector<double>> sp_matrix(n, vector<double>(n));         // Matriz de caminhos mais curtos para todos os nós
    double sol_value = 0.0;                                         // Valor da solução inicial
    vector<int> hub_list;                                           // Lista de hubs ativos
    vector<vector<double>> l(n, vector<double>(n));
    vector<vector<double>> le(n, vector<double>(n));

    // Prepara lista de hubs e adiciona o custo de instalação ao valor da solução
    for (int k = 0; k < n; k++)
    {
        if (h[k] >= (1 - EPSILON))
        {
            hub_list.push_back(k);
            sol_value -= s[k];
        }
    }

    // ==============================================
    // 1. Calcular o caminho mais curto apenas entre hubs
    // ==============================================

    #pragma omp parallel for collapse(2) reduction(-:sol_value)
    for (size_t i = 0; i < hub_list.size(); ++i)
    {
        for (size_t j = 0; j < hub_list.size(); ++j)
        {
            if (i != j)
            {
                int k = hub_list[i];
    ...
    // ==============================================
    // 2. Atualiza a matriz de caminhos mais curtos entre hubs
    // ==============================================
    ...
    for (size_t i = 0; i < hub_list.size(); ++i)
    {
        for (size_t j = 0; j < hub_list.size(); ++j)
        {
            for (size_t k = 0; k < hub_list.size(); ++k)
            {
                if (i != j && j != k && i != k)
                {
                    int m = hub_list[i];
                    int n = hub_list[j];
                    int l = hub_list[k];
                    double cost_aux = hub_net_sp_matrix[m][n] + hub_net_sp_matrix[n][l];
    ...
    // ==============================================
    // 3. Calcular o caminho mais curto para todos os pares de nós
    // ==============================================
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            ...
            for (size_t k_index = 0; k_index < hub_list.size(); ++k_index)
            {
                int k = hub_list[k_index];
                sp_matrix[i][j] = min(c[i][k] + c[k][j], sp_matrix[i][j]);
                for (size_t m_index = 0; m_index < hub_list.size(); ++m_index)
                {
                    int m = hub_list[m_index];
                    if (k != m)
                    {
...
void Heuristicas::vizinhanca1()
{
    ...
    if (!hubs_inativos.empty())
    {
        ...
        #pragma omp parallel for private(f_avaliacao, posicao) shared(h_linha, valor_solucao_linha)
        for (int j = 0; j < u; j++)
        {
            if (j == 0)
            {
...
void Heuristicas::vizinhanca2()
{
    ...
    for (int k = 0; k < n; k++)
    { // coletando arcos entre hubs ativos
        for (int m = 0; m < n; m++)
        {
            if (m != k)
            {
...
void Heuristicas::vizinhanca3()
{
    ...
    if (hubs_ativos.size() > 1)
    { // conjunto de arcos entre hubs possíveis
...
void Heuristicas::vizinhanca5()
{
    ...
    { // Caso não tenha nenhum hub instalado, será ativado um hub aleatoriamente e a vizinhança5 é reiniciada
        posicao = rand() % hubs_inativos.size();
        h[hubs_inativos[posicao]] = 1;
...
void Heuristicas::SmartILS(int iter_max)
{
    ...
    //==========Loop Principal SmartILS com Perturbação Variável==========//

    while (iter < iter_max)
    {
        int niveis[] = {1, 2, 3}; // Exemplo de diferentes níveis de perturbação

        // Variáveis para armazenar a melhor solução em cada perturbação
        vector<double> solucoes_perturbadas(3, -1e9); // Três níveis de perturbação
        vector<vector<double>> h_perturbados(3), z_perturbados(3), e_perturbados(3);
        ...
        for (int p = 0; p < 3; p++)
        {
            ...
            // Aplica a perturbação e executa VND
            Perturbacao(nivel);
            VND();

            // Armazena a solução perturbada
            solucoes_perturbadas[p] = valor_solucao;
        ...
        // Seleciona a melhor solução entre as perturbações testadas
        double melhor_solucao = valor_solucao_star;
        int melhor_perturbacao = -1;

        for (int p = 0; p < 3; p++)
        {
            if (solucoes_perturbadas[p] > melhor_solucao)
            {
                melhor_solucao = solucoes_perturbadas[p];
                melhor_perturbacao = p;
            }
        }

        // Atualiza com a melhor solução encontrada
        if (melhor_perturbacao != -1)
        {
            h = h_perturbados[melhor_perturbacao];
            z = z_perturbados[melhor_perturbacao];
            e = e_perturbados[melhor_perturbacao];
            valor_solucao = melhor_solucao;
        }
